...
Last message being composed seems to have been aborted.
    ...
    j <:s; -d> zj			!* Get rid of deletes that may!
					!* have accidently appeared!
...
This routine knows about the format of the status line in fair detail.!
 ...
 m(m.m& Declare Load-Time Defaults)
	...
	* Non-0 causes autolabeling from subject fields: 0
     Babyl Convert TNX Flags to Labels,
        If nonzero, convert well known TNX MAIL.TXT flags: 0

...
*** EOOH ***"n :i*Mail file does not end with Control-_ fsErrw'
 ...
 q2&(qBabyl Convert TNX Flags to Labels"'n)"n
...

 ...
 [0[1[2[3[4				!* save registers!
 QBabyl Keep TNX Received DateU0	!* Nonzero to add Rcvd-Date line. !
 QBabyl Convert TNX Flags to LabelsU3	!* Nonzero preserves flags!
 j < .-z; .u2				!* 2: start of message!
     :fb,; \u1 0,1a-;@:;		!* 1: Message length.!
     Q3"N				!* If preserving flags as labels!
        c :f  @; x4 '			!* 4: saved MAIL.TXT flags!
...
					!* Add CRLF. !
	.U2 L '				!* And setup new start of message. !
     ...
     Q3"N				!* If preserving flags as labels!
	IX-Babyl-TNX-Bit-Flags: g4 '	!* Put in as header line for now!
...


!& Autolabel TNX Bit Labels:! !S Post-Babylizing conversion routine.!
!*  We only come here when we are known to have a babylized TNX file and we
    know that the user has enabled preservation of the TNX numeric flags as a
    fake header (since TNX conversion happens within RMAIL conversion).  We
    remove the fake headers as we go along, and frob the labels to the nearest
    standard equivalents of the well-known TNX flag bits.

    If you think this code looks like & Autolabel Subject Labels, you are
    right.

    The following is from MM.MAC.1112 and describes the well-known flag values
    in MAIL.TXT format files as of 21-Sep-86.  We preserve these as an
    additional header line, since this routine is converting to RMAIL format
    and we cant label messages directly.  This should be converted by our
    calling routine; I suppose we could use autolabeling instead if it really
    mattered.
    ...
    See comment in & Autolabel Subject Labels regarding bound frobbing.
!

 [0 [1 .[2 fnq2j			!* 2: auto-restoring point.!
 0f[VB 0f[VZ				!* Widen and preserve bounds!
 m.m& Label Babyl Message[L		!* L: Labeler.!
 <  0,1a-@:; l .u1			!* 1: Start of message!
    @:f:fb

;					!* Find end of headers!
    q1,.:fb
X-Babyl-TNX-Bit-Flags: ;		!* Find magic header!
    8f[IBase w \u0 w f]IBase		!* 0: TNX flag bits in octal!
    :f  @:; 0kk			!* Remove magic header!
    1,(q0&1"'n*2+1)mLunseen    w q1j	!* M%SEEN==1  ;Message has been seen!
    1,(q0&2"'e*2+1)mLdeleted   w q1j	!* M%DELE==2  ;Message is deleted!
    1,(q0&4"'e*2+1)mLRemindNow w q1j	!* M%ATTN==4  ;Message wants attention!
    1,(q0&8"'e*2+1)mLanswered  w q1j	!* M%RPLY==10 ;Message replied to!
    :s;				!* Skip to end of message!
 >					!* Done or lost!
 z-."e '				!* Done!

 :i*Babyl bug: Unable to convert TNX bits to labels?fserr
 ...
 * Fill Column:78
 * Comment Column:40
 * Compile Command:m(m.mGenerate Library)emacs:babylemacs:babyl emacsemacs:babylm emacsemacs:bboard emacsemacs:bbsite emacs
 * End:
 * *!
